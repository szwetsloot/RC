...
var number_boats = 3; // number of boats visible on screen
...
Boat.prototype = {
		...
		'moveBoat': function(target_y, target_x, direction) { 
			...
			calcTrail(target_x,target_y,this.id); // draw the trail of the boat
...
function createBoats(){
	...
	for ( i = 0; i < number_boats; i++ ){
		
		var boat = boats[i] = new Boat();
		
		var $new_boat = $('#dummy-boat').clone().appendTo('#boat-container');
		
		$new_boat.attr('id','boat-'+i);
		
		var x = $('#boat-'+i).position();
...
function startWatch(){ 
	...
	if( minutes > end_animation ) run = 0; 
